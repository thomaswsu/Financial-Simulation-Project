...
delta.hedge.CRN <- function(M,N,S0,K,r,sigma,t,mu,alpha,b,volvol,V0,call, barrier = S0 + 10){
 ...
 #disc <- exp(-r*dt*rev(c(0:N)))
  ...
  H.perf <- sqrt(var(PV)) / P.Barrier
  #outlist <- list("H.perf"=H.perf,"PV"=PV,"P.Euro"=P.Euro, "deltas.A"=deltas.A,"CF"=CF)
  outlist <- list("H.perf"=H.perf,"PV"=PV,"P.Barrier"=P.Barrier, "deltas.A"=deltas.A,"CF"=CF)
  return(outlist)
}
...
V0 <- 0.4
set.seed(2021)
CRN.1 <- delta.hedge.CRN(M,N,S0,K,r,sigma,t,mu,alpha,b,volvol,V0,call=2) # CHANGE THIS DEPENDING ON OPTION
...
PV <- c(NA)
for (j in 1:length(N)){
  tmp <-  delta.hedge.CRN(M,N[j],S0,K,r,sigma,t,mu,alpha,b,volvol,V0,call=2) # CHANGE THIS DEPENDING ON OPTION
  H[j] <- tmp$H.perf
  PV[j] <- mean(tmp$PV)
}
print(H)
